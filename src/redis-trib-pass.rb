...
class RedisTrib
    ...
    def import_cluster_cmd(argv,opt)
        ...
        src_host,src_port,src_password = source_addr.split(":")
        
        if src_password != nil
            source = Redis.new(:host =>src_host, :port =>src_port, :password => src_password)
        else
...
ALLOWED_OPTIONS={
    "create" => {"auth" => true, "replicas" => true},
    "add-node" => {"auth" => true, "slave" => false, "master-id" => true},
    "import" => {"auth" => true, "from" => :required, "copy" => false, "replace" => false},
    "reshard" => {"auth" => true, "from" => true, "to" => true, "slots" => true, "yes" => false, "timeout" => true, "pipeline" => true},
    "rebalance" => {"auth" => true, "weight" => [], "auto-weights" => false, "use-empty-masters" => false, "timeout" => true, "simulate" => false, "pipeline" => true, "threshold" => true},
    "fix" => {"auth" => true, "timeout" => MigrateDefaultTimeout},
    "check" => {"auth" => true}, 
    "info" => {"auth" => true}, 
    "del-node" => {"auth" => true}, 
    "set-timeout" => {"auth" => true}, 
    "call" => {"auth" => true}                       
}
